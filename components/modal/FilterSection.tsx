  ...
  return (
    <div className="border-b border-border">
      <motion.button
        ...
        className="flex w-full items-start gap-2 px-4 py-3 text-left hover:bg-[#F9FAFB]"
        whileTap={{ scale: 0.99 }}
      >
        ...
        {isOpen ? (
          ...
          >
            ...
            <input
              ...
              onChange={(event) => onChange(event.target.value)}
              placeholder={placeholder}
              className="h-9 w-full rounded-lg border border-border bg-background px-3 text-[13px] text-text-primary placeholder:text-text-secondary focus:outline-none focus:ring-1 focus:ring-primary-blue"
            />
          </motion.div>
        ) : null}
      </AnimatePresence>
...
export function FilterPanel({
  ...
  return (
    <div className="ml-8 mt-8 mb-[18px] flex h-[668px] w-[319px] shrink-0 flex-col bg-background">
      {title || headerAction ? (
        <div className="flex shrink-0 items-center justify-between px-4 pb-3 pt-4">
          {title ? (
            <h2 className="text-[18px] font-semibold leading-[25.2px] text-text-primary">
              {title}
            </h2>
          ) : (
            <span />
